...
zip_count=coll.defaultdict(int)
...
print "The gini index of the candidates, not split by zip codes, is "+str(gini)

split_gini = sum( (1-sum((zip_dicts[code][cand]/total)**2 for cand in zip_dicts[code]))*sum(zip_dicts[code].values())/total for code in zip_dicts)  # weighted average of the Gini Indexes using candidate names, split up by zip code
##/
